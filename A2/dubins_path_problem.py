...
class RRT_dubins_problem:
    ...
    class Node:
        ...
        cost    - The total length of path to be travelled to reach
                  this node from the start location
        path_x  - a list of x coordinates of all vehicle states included in the
                  dubins-type path joining parent node to the current node
        path_y  - a list of y coordinates of all vehicle states included in the
                  dubins-type path joining parent node to the current node
        path_yaw- a list of yaw values of all vehicle states included in the
                  dubins-type path joining parent node to the current node

        NOTE: x, y, yaw together constitute the state of the vehicle in the current node
        NOTE: path_x, path_y, path_yaw together constitute the complete path of the vehicle
    ...
    def __init__(self, start, goal, obstacle_list, map_area, max_iter=10000):
        """
        Init Parameter
    ...
    def propagate(self, from_node, to_node):
        """
        PROPAGATE is a helper function that creates a new_node at the state
        ...
        to_node   - (Node) child node describing the goal state of this propagation
    ...
    def draw_graph(self):
        ...
        plt.axis(
            [self.x_lim[0] - 2, self.x_lim[1] + 2, self.y_lim[0] - 2, self.y_lim[1] + 2]
        )
        plt.grid(True)
    ...
    def plot_start_goal_arrow(self):
        ...
        dubins_path_planning.plot_arrow(self.goal.x, self.goal.y, self.goal.yaw)
...
def check_path(rrt_problem, path_node_list: list):
    ...
    a valid path which satisfies the following conditions:
