...
## New Custom Strategy
This strategy prioritizes the nearest reachable frontier while adding a logarithmic bonus for larger clusters, ensuring the robot doesn't ignore massive unknown areas for tiny, insignificant gaps. It differs from the standard "nearest" approach by using a dynamic scoring formula (Cost -20.log(Size) ) and a faster cluster-cell fallback that avoids the heavy BFS search if a frontier's center is inside a wall. This makes it more robust in cave environments where narrow passages often cause traditional centroid-based planning to fail.
